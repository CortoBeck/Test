...
def update_book(book_id, title, author, edition, remarque, genre, resume):
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        """
        UPDATE books
        SET title = ?, author = ?, edition = ?, remarque = ?, genre = ?, resume = ?
        WHERE id = ?
    """,
        (title, author, edition, remarque, genre, resume, book_id),
    )
    conn.commit()
    conn.close()
